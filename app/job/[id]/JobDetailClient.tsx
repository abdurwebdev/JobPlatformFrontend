...
export default function JobDetailClient({ initialData }: JobDetailClientProps) {
  // 1. Force the client tab title to change on the exact frame the data loads
  if (typeof window !== "undefined" && initialData?.title) {
    document.title = `${initialData.title} – ${initialData.company_name || "Job Platform"}`;
  }

  const [job, setJob] = useState<JobData | null>(initialData);

  // Sync state if the initialData changes during dynamic client navigation shifts
  useEffect(() => {
    setJob(initialData);
  }, [initialData]);
